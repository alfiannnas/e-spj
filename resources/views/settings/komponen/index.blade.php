...
<head>
    ...
    <title>Daftar Komponen - {{ config('app.name', 'Laravel') }}</title>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="bg-white shadow-sm border-b border-gray-200 px-8 py-4">
                <div class="flex items-center justify-between">
                    <h2 class="text-2xl font-bold text-gray-900">Daftar Komponen</h2>
            ...
            <div class="flex-1 overflow-auto">
                <div class="max-w-7xl mx-auto px-8 py-8">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-gray-900">Daftar Komponen</h3>
                        <a href="{{ route('komponen.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center gap-2">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="bg-white rounded-lg shadow overflow-hidden">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode Komponen</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Komponen</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">RO</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse ($komponens as $komponen)
                                        <tr class="hover:bg-gray-50 transition duration-150">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $komponen->kode_komponen }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $komponen->nama_komponen }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                                @if($komponen->ro)
                                                    <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">{{ $komponen->ro->kode_ro }}</span>
                                                @else
                                                    <span class="text-gray-400">-</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-2">
                                                <a href="{{ route('komponen.edit', $komponen->id) }}" class="text-blue-600 hover:text-blue-900 transition duration-200">
                                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                                ...
                                                <form action="{{ route('komponen.destroy', $komponen->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus?');">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                                ...
                                                <p class="text-lg font-medium">Tidak ada data Komponen</p>
                                                <p class="text-sm">Mulai dengan membuat Komponen baru</p>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                        ...
                        @else
                            <div class="px-6 py-12 text-center">
                                ...
                                <p class="text-lg font-medium text-gray-900">Tidak ada data Komponen</p>
                                <p class="text-sm text-gray-500">Mulai dengan membuat Komponen baru</p>
                            </div>
                        @endif
                    </div>
                </div>
