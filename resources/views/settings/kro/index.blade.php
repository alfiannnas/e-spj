...
<head>
    ...
    <title>KRO Management - {{ config('app.name', 'Laravel') }}</title>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="bg-white shadow-sm border-b border-gray-200 px-8 py-4">
                <div class="flex items-center justify-between">
                    <h2 class="text-2xl font-bold text-gray-900">KRO Management</h2>
            ...
            <div class="flex-1 overflow-auto">
                <div class="max-w-7xl mx-auto px-8 py-8">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-gray-900">Daftar KRO</h3>
                        <a href="{{ route('kro.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center gap-2">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="bg-white rounded-lg shadow overflow-hidden">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode KRO</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama KRO</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse ($kros as $kro)
                                        <tr class="hover:bg-gray-50 transition duration-150">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $kro->kode_kro }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $kro->nama_kro }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                                @if($kro->program)
                                                    <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">{{ $kro->program->kode_kegiatan }}</span>
                                                @else
                                                    <span class="text-gray-400">-</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-2">
                                                <a href="{{ route('kro.edit', $kro->id) }}" class="text-blue-600 hover:text-blue-900 transition duration-200">
                                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                                    </svg>
                                                </a>
                                                <form action="{{ route('kro.destroy', $kro->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus KRO ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="px-6 py-4 text-center text-gray-500">Tidak ada data KRO</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        @else
                            <div class="p-8 text-center">
                                ...
                                <p class="text-gray-600">Belum ada data KRO</p>
                                <a href="{{ route('kro.create') }}" class="mt-4 inline-block text-blue-600 hover:text-blue-900 font-medium">
                                    Mulai dengan menambahkan KRO baru
                                </a>
                            </div>
                        @endif
                    </div>
