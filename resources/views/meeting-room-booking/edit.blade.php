...
<head>
    ...
    <title>Edit Peminjaman Ruang Rapat - {{ config('app.name', 'Laravel') }}</title>
                    ...
                    <div class="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
                        <div class="overflow-x-auto">
                            <div class="max-w-5xl mx-auto">
                                ...
                                <!-- Header -->
                                <div class="mt-9 mb-6">
                                    <h2 class="text-2xl font-bold text-gray-900">Edit Peminjaman Ruang Rapat</h2>
                                    <p class="text-sm text-gray-600">Perbarui data peminjaman ruang rapat dengan lengkap dan benar</p>
                                </div>

                                <!-- Form -->
                                <div class="bg-white rounded-xl shadow border border-gray-200 p-8 mx-auto mb-20 ">
                                    <form action="{{ route('meeting-room-booking.update', $meetingRoomBooking) }}" method="POST" class="space-y-6">
                                        @csrf
                                        @method('PUT')
                                        <div class="grid grid-cols-2 gap-6">
                                            <div>
                                                <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Peminjaman</label>
                                                <input type="date" name="booking_date" value="{{ old('booking_date', $meetingRoomBooking->booking_date ? \Carbon\Carbon::parse($meetingRoomBooking->booking_date)->format('Y-m-d') : '') }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="time" name="booking_time" value="{{ old('booking_time', $meetingRoomBooking->booking_time ? \Carbon\Carbon::parse($meetingRoomBooking->booking_time)->format('H:i') : '') }}"
                                            ...
                                            <div class="md:col-span-2">
                                                ...
                                                <textarea name="booking_purpose" rows="4"
                                                    ...
                                                    placeholder="Masukkan nama kegiatan yang akan dilaksanakan">{{ old('booking_purpose', $meetingRoomBooking->booking_purpose) }}</textarea>
                                        ...
                                        <div class="flex justify-end gap-4 pt-6 border-t border-gray-100">
                                            ...
                                            <button type="submit"
                                                class="px-6 py-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold shadow hover:from-blue-700 hover:to-indigo-700">Simpan Perubahan</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
